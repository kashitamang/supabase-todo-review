...
export async function createTodo(todo) {
    // create a single incomplete todo with the correct 'todo'
    // property for this user in supabase
...
export async function completeTodo(id) {
    // find the and update (set complete to true),
    const response = await client.from('todos').update({ complete: true }).eq('id', id);
    // the todo that matches the correct id
    return response;
    // once you have a response from supabase, comment this back in:
    // return checkError(response);
}
